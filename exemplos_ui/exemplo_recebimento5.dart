...
class ItemNota {
  ...
  bool get temDivergenciaEsperadoVsRecebido {
    return conferido > 0 && conferido != quantidade;
  }

  bool get temDivergenciaRecebidoVsLotes {
    if (lotes == null || lotes!.isEmpty) return false;
    final somaLotes = lotes!.fold<int>(0, (sum, lote) => sum + lote.conferido);
    return (conferido > 0 || somaLotes > 0) && conferido != somaLotes;
  }

  bool get temDivergencia {
    return temDivergenciaEsperadoVsRecebido || temDivergenciaRecebidoVsLotes;
  }

  String get mensagemDivergencia {
    if (temDivergenciaEsperadoVsRecebido) {
      return 'Esperado: $quantidade | Recebido: $conferido';
    }
    if (temDivergenciaRecebidoVsLotes) {
      final somaLotes = lotes!.fold<int>(
        0,
        (sum, lote) => sum + lote.conferido,
      );
      return 'Recebido: $conferido | Soma lotes: $somaLotes';
    }
    return '';
  }
}
...
class NotaFiscal {
  ...
  List<ItemNota> get itensComDivergencia {
    return itens.where((item) => item.temDivergencia).toList();
  }
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void _tentarFinalizarConferencia(bool isTablet) {
    ...
    if (nota.temDivergencias) {
      _mostrarDialogDivergencias(nota, isTablet);
    } else {
      _finalizarConferencia(isTablet);
    }
  }

  void _mostrarDialogDivergencias(NotaFiscal nota, bool isTablet) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              Text(
                'Foram encontradas divergências em ${nota.itensComDivergencia.length} ${nota.itensComDivergencia.length == 1 ? 'item' : 'itens'}:',
                style: const TextStyle(fontWeight: FontWeight.bold),
              ),
              const SizedBox(height: 16),
              ...nota.itensComDivergencia.map((item) {
                return Padding(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).pop();
              _finalizarConferencia(isTablet, comDivergencia: true);
            },
  ...
  void _finalizarConferencia(bool isTablet, {bool comDivergencia = false}) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          comDivergencia
        ...
        backgroundColor: comDivergencia ? Colors.orange : Colors.green,
      ),
    );
                ...
                children: [
                  ...
                  Expanded(
                    child: ElevatedButton(
                      onPressed: () => _tentarFinalizarConferencia(isTablet),
                      style: ElevatedButton.styleFrom(
                        padding: const EdgeInsets.symmetric(vertical: 16),
...
class _ItemConferenciaCardState extends State<ItemConferenciaCard> {
  ...
  Widget build(BuildContext context) {
    ...
    // Define cor da borda baseado no status
    ...
    if (widget.item.temDivergencia) {
      ...
      borderWidth = 2;
    } else if (widget.item.conferido > 0 && !widget.item.temDivergencia) {
  ...
  Widget _buildStatusIcon() {
    final bool conferidoOk =
        !widget.item.temDivergencia && widget.item.conferido > 0;
